...
from app.crosscutting.authorization import getAuthenticatedUser
from app.security.domain.model.user import User, Role
from app.crosscutting.authorization import authorizeRoles
from app.mapping.appointment_mapper import AppointmentMapper
...
async def createAppointment(
    ...
    medic_service = Depends(Provide[Container.medicService])
):
    ...
    model = AppointmentMapper.requestToModel(
        request,
        medic_full_name=medic.user.full_name,
...
@router.get("/schedule/{medic_id}/{day}", response_model=dict,
...
def get_schedule_status(
    medic_id: int,
    day: date,
    appointment_service: AppointmentService = Depends(Provide[Container.appointmentService]),
    medic_service = Depends(Provide[Container.medicService])
):
