# app/domain/persistence/medic_repository.py
...
class MedicRepository:
    ...
    def find_by_id(self, medic_id: int) -> Optional[Medic]:
        return self.session.query(Medic).filter(Medic.id == medic_id).first()


    def find_by_user_id(self, user_id: int) -> Optional[Medic]:
        """Busca un médico por el ID del usuario relacionado."""
        return self.session.query(Medic).filter(Medic.user_id == user_id).first()
    ...
    def get_by_specialty(self, specialty: str) -> List[Medic]:
        ...
        return self.session.query(Medic).filter(
