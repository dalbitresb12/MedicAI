...
class MedicService:
    ...
    def create_profile_for_medic(
        self, user_id: int, data: MedicCreateRequest
    ) -> MedicResponse:
    ...
    def list_by_specialty(self, specialty: str) -> list[MedicResponse]:
        return [
            MedicMapper.to_response(m)
            for m in self.repository.get_by_specialty(specialty)
        ]

    def list_by_name(self, name: str) -> list[MedicResponse]:
        return [
            MedicMapper.to_response(m)
            for m in self.repository.get_by_name(name)
        ]
    ...
    def update_profile_for_medic(
        self, user_id: int, data: MedicUpdateRequest
    ) -> MedicResponse:
        medic = self.repository.find_by_user_id(user_id)
        if not medic:
            raise ValueError("El perfil del médico no existe.")
