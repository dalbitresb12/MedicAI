...
from app.security.schemas.request.update_password_request import (
...
@router.put(
    ...
    dependencies=[
        Depends(authorizeRoles([Role.PATIENT, Role.ADMIN, Role.MEDIC]))
    ],
)
@inject
async def updateMyUser(
    request: UpdateUserRequest,
...
async def updateMyPassword(
    ...
    user = userService.updatePasswordById(
        authenticatedUser.id, request.password
    )
    return UserMapper.modelToResponse(user)
