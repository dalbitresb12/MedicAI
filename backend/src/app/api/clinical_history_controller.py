...
from app.schemas.request.create_clinical_history_request import (
    CreateClinicalHistoryRequest,
)
from app.schemas.response.clinical_history_response import (
...
router = APIRouter(prefix="/api/v1/history", tags=["Clinical History"])
...
def create_history(
    ...
    service: ClinicalHistoryService = Depends(
        Provide[Container.clinicalHistoryService]
    ),
):
    return service.create(
        request,
        medic_id=current_user.id,
        medic_full_name=current_user.full_name,
    )


@router.get(
    "/by-patient-email/{email}",
